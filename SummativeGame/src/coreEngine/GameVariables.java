...
public class GameVariables {
    ...
    //stores the name of the player, starts as default, stays that way if not changed by the player in the options menu
    private static String playerName = defaultHighScoreName;

    //stores the diameter of the flashlight in pixels
    private static final int FlashlightDiameter = 1280;
    //stores the name of the level file, can be changed by the player in the options menu
    private static String levelFile = "LevelZero";
    //stores whether it is night or not, can also be changed by the player in the options menu
    private static boolean isNight = true;

    //stores the amount of hunters and runners to render, changed by the difficulty settings
    private static int runnersToRender = 3;
    private static int huntersToRender = 0;
    //stores the damage dealt by the hunters
    private static int hunterDamage = 1;
    //stores the maximum different types of tiles that can be creates in the game
    private static final int MAX_GAME_TILES = 64;
    private static final int STANDARD_TILE_DIAMETER = 64;
    //VARIABLES BELOW ARE IDENTIFIERS AND FILE LOCATIONS FOR GAME TILES
